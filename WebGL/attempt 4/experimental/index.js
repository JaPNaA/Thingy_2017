...
function render(X, programInfo, buffers) {
    ...
    X.uniform1f(programInfo.uniform.time, false, performance.now());
    X.uniform2f(programInfo.uniform.mouse, false, [programInfo.mouse.x, programInfo.mouse.y]);
    X.uniform2f(programInfo.uniform.res, false, [X.canvas.width, X.canvas.height]);
...
var program, dt, buffers;

/**
 * main function, everything happens here
 */
function main() {
    program = initializeShaderProgram(X, VERT, FRAG); // create program with vertex and fragment shaders
    dt = { // programInfo, could be a class
        ...
        uniform: {
            ...
            time: X.getUniformLocation(program, "time"),
            mouse: X.getUniformLocation(program, "mouse"),
            res: X.getUniformLocation(program, "resolution")
        },
        mouse: {
            x: 0,
            y: 0
    ...
    addEventListener("mousemove", function (e) {
        dt.mouse.x = e.clientX;
        dt.mouse.y = e.clientY;
    });
}
